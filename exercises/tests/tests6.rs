...
unsafe fn raw_pointer_to_box(ptr: *mut Foo) -> Box<Foo> {
    ...
    // We can safely reconstruct the `Box<Foo>` from the raw pointer.
    unsafe {
        Box::from_raw(ptr)
    }
}
...
mod tests {
    ...
    #[test]
    fn test_success() {
        // Set the `b` field in the data box to match the assertion.
        ...
        // Set `b` field correctly for the assertion.
